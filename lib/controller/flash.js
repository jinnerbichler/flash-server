...
const getChannel = FlashChannel.getChannel;
const updateChannel = FlashChannel.updateChannel;
const FlashObject = FlashChannel.Model;
...
router.post('/init', validate(schemas.init), asyncMiddleware(async (req, res, next) => {
    ...
    // save flash channel
    let flashObj = new FlashObject({data: flash});
    await flashObj.save(function (err) {
...
router.post('/multisignature/:channelId', validate(schemas.multisignature), asyncMiddleware(async (req, res, next) => {
    ...
    const flash = (await getChannel(channelId)).data;
    ...
    // update flash object
    await updateChannel(channelId, flash);
...
router.post('/settlement/:channelId', validate(schemas.settlement), asyncMiddleware(async (req, res, next) => {
    ...
    let flash = (await getChannel(req.params.channelId)).data;
    flash.flash.settlementAddresses = settlementAddresses;
    await updateChannel(req.params.channelId, flash);
...
router.post('/fund/:channelId', asyncMiddleware(async (req, res, next) => {
    ...
    const flash = (await getChannel(req.params.channelId)).data; // ToDo: validate flash object (e.g. multisignatures, settlement addresses, ...)
...
router.post('/finalize/:channelId', asyncMiddleware(async (req, res, next) => {
    ...
    IOTA.api.sendTrytes(bundleTrytes, flash.depth, IRI_MIN_WEIGHT, {}, function (error, finalTransactions) {
        ...
        updateChannel(req.params.channelId, flash).then(() => {
            res.json({
...
router.get('/balance/:channelId', asyncMiddleware(async (req, res, next) => {
    const flash = (await getChannel(req.params.channelId)).data;
